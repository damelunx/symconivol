...
```{r ind_norm_from_constr_eigval-3, echo=FALSE}
...
### The rate function of the index probabilities
...
```{r rate-fct, fig.width = 7, fig.height=4, echo=FALSE}
R <- rate()
pL <- list(
...
```{r approx-indprobrat, fig.width = 7, fig.height=6, echo=FALSE}
plotIndP <- list()
IndP <- list()
j <- 0
R <- rate()
...
```{r est-var-PhiInd, echo=FALSE}
include_graphics("var_PhiInd.png", dpi=img_dpi)
```
...
```{r illustr-dim-norm-cm_artif, fig.width = 7, fig.height=12, echo=FALSE}
plotsPB <- list()
plotPhiDim <- list()
j <- 0
L <- leigh()
...
for (beta in c(1,2,4)) {
    for (n in c(6,10,20)) {
        ...
        for (k in (0:d)) {
            for (r in 0:n) {
                if (r>=pat$r_low(k) & r<=pat$r_upp(k)) {
                    B[1+k,1+r] <- exp(-d*(k/d-L$lkup_kappa(r/n))^2/2/C/(1-abs(2*r/n-1)))
                }
            }
            B[,1+r] <- B[,1+r]/sum(B[,1+r])
        }
        A <- matrix(0,d+1,n+1)
        A[1,1] <- 1
        A[d+1,n+1] <- 1
...
```{r disp-lim-PhiDim, echo=FALSE}
include_graphics("lim_PhiDim.png", dpi=img_dpi)
```


## Deriving the asymptotics for index constrained curvature measures
...
```{r lim_dens_MNSV, fig.width = 7, fig.height=4, echo=FALSE, warning=FALSE}
bnd_a <- function(a) {
    L <- L_a(a)
...
We can use this function to express the squared norm of the positive part of $x$:
  \[ \|x_+\|^2 = n\int_0^\infty \big(\sqrt{\beta}nt\big)^2 g(x;t) dt
...
More precisely, $\delta(m,n,r)$ with $1\leq m\leq n-1$ is nonzero iff
...
```{r illustr-marg-ad, fig.width = 7, fig.height=8, echo=FALSE}
plotsAD <- list()
j <- 0
for (n in c(4,8,14)) {
    pat <- pat_bnd(1,n)
...
```{r illustr-norm-ad, fig.width = 7, fig.height=8, echo=FALSE}
plotsAD <- list()
j <- 0
for (n in c(5,8,14)) {
    beta <- 1
